...
export function AppLayout({ children, user, onSync, syncing }: Props) {
  ...
  const sidebarContent = (
    <div className="flex h-full flex-col justify-between bg-white px-4 py-6 border-r border-zinc-200">
      <div className="space-y-6">
        {/* Brand logo */}
        <div className="flex items-center justify-between">
          <TransitionLink href="/dashboard" className="flex flex-col">
              ...
              <TransitionLink
                ...
                className={cn(
                  ...
                  isActive
                    ...
                    : "text-zinc-650 text-zinc-600 hover:bg-zinc-50 hover:text-zinc-900"
                )}
                onClick={() => setSidebarOpen(false)}
              >
                <item.icon className={cn("h-5 w-5 shrink-0", isActive ? "text-indigo-600" : "text-zinc-400")} />
                {item.name}
              </TransitionLink>
            );
          })}
      ...
      <div className="space-y-4 pt-4 border-t border-zinc-100">
        <div className="flex items-center gap-3">
          {user.image ? (
            ...
            <img
              src={user.image}
              alt=""
              className="h-9 w-9 rounded-full ring-2 ring-zinc-100"
            />
          ) : (
        ...
        <button
          ...
          className="flex w-full items-center gap-3 px-3 py-2 text-sm font-semibold text-zinc-650 text-zinc-600 hover:text-red-600 hover:bg-red-50 rounded-xl transition cursor-pointer"
        >
  ...
  return (
    <div className="min-h-screen bg-zinc-50 flex">
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:pl-64">
        ...
        <header className="flex h-16 items-center justify-between border-b border-zinc-200 bg-white px-4 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-indigo-600 leading-none">
                Weeklinks
              </span>
              <span className="text-xs font-bold text-zinc-950 mt-0.5">
                Topic Tracker
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {onSync && (
              <button
                ...
                onClick={onSync}
                disabled={syncing}
                className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 px-2.5 py-1.5 text-xs font-semibold text-white transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-60 cursor-pointer"
              >
            ...
            <div className="hidden items-center gap-2 rounded-full border border-zinc-200 bg-zinc-50 px-2.5 py-1 lg:flex">
              {user.image ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={user.image}
              ...
              <span className="max-w-[100px] truncate text-xs text-zinc-700 font-medium">
            ...
            <button
              type="button"
              onClick={handleSignOut}
              className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-zinc-700 transition hover:bg-zinc-50 cursor-pointer"
              aria-label="Sign out"
            >
        ...
        <main className="flex-1 overflow-y-auto">
          {isPending ? (
            <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-center animate-pulse">
              ...
              <p className="text-sm font-bold text-zinc-950 mt-4 tracking-tight">
                Loading workspace
              </p>
      ...
      {showSignOutConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          {/* Backdrop overlay */}
          <div
            className="fixed inset-0 bg-zinc-950/45 bg-zinc-950/40 backdrop-blur-xs transition-opacity duration-300"
            onClick={() => setShowSignOutConfirm(false)}
          />
          {/* Modal Container */}
          <div className="relative bg-white rounded-2xl border border-zinc-200 p-5 max-w-sm w-full shadow-2xl flex flex-col gap-4 z-50 transform transition-all duration-300 scale-100">
            <div>
              <h3 className="text-base font-bold text-zinc-950">Sign Out</h3>
            ...
            <div className="flex items-center justify-end gap-2.5">
              <button
                type="button"
                onClick={() => setShowSignOutConfirm(false)}
                className="px-3.5 py-2 border border-zinc-200 bg-white hover:bg-zinc-50 rounded-xl text-xs font-semibold text-zinc-700 transition cursor-pointer"
              >
                Cancel
              </button>
              <button
